        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold transition-transform transform active:scale-95 text-xs flex items-center gap-1.5 shadow-lg shadow-emerald-500/20"
            >
                ...
                {products?.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-3 text-center flex items-center justify-center gap-2">
                      <button onClick={() => handleEditProduct(p)} className="px-2 py-1 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg text-[10px] transition-colors">
                        แก้ไข
                      </button>
                      <button onClick={() => handleToggleProductStatus(p)} className={`px-2 py-1 rounded-lg text-[10px] transition-colors ${
              ...
              <tbody className="divide-y divide-slate-100">
                {stores?.map((s: any) => (
                  <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-3 text-center flex items-center justify-center gap-2">
                      <button onClick={() => handleEditStore(s)} className="px-2 py-1 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg text-[10px] transition-colors">แก้ไข</button>
                      <button onClick={() => { setTransferStoreId(s.id); setTransferTargetUserId(s.assignedUserId || ''); setShowTransferModal(true); }} className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-[10px] transition-colors">โอนย้ายเซลล์</button>
                      <button onClick={() => handleDeleteStore(s.id)} className="px-2 py-1 border border-rose-200 hover:bg-rose-50 text-rose-600 rounded-lg text-[10px] transition-colors">ลบ</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody className="divide-y divide-slate-100">
                {users?.map((u: any) => (
                  <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-3 text-center flex items-center justify-center gap-2">
                      <button onClick={() => handleEditUser(u)} className="px-2 py-1 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg text-[10px] transition-colors">แก้ไข</button>
                      <button onClick={() => handleToggleUserStatus(u)} className={`px-2 py-1 rounded-lg text-[10px] transition-colors ${
